    ...
    stage('Pick CAR') {
      steps {
        script {
          ...
          if (!env.CAR_FILE) {
            error("No .car found in repo. Commit it (e.g. HelloWorldService_1.0.0.car) or build it in pipeline.")
          }
        ...
        sshagent(credentials: ["${env.SSH_CRED}"]) {
          sh(label: 'Deploy to DEV', script: '''
            ...
            ssh -o StrictHostKeyChecking=no ubuntu@${DEV_HOST} \\
              "CONTAINER='${CONTAINER}' MI_CARBONAPPS_DIR='${MI_CARBONAPPS_DIR}' REMOTE_STAGE_DIR='${REMOTE_STAGE_DIR}' SMOKE_URL='${SMOKE_URL}' bash -s" <<'EOS'
              set -euo pipefail

              CAR_NAME="$(ls -1 ${REMOTE_STAGE_DIR}/*.car | xargs -n1 basename | head -n 1)"
              ...
              # smoke test (fails build if non-200)
      ...
      steps {
        sshagent(credentials: ["${env.SSH_CRED}"]) {
          sh(label: 'Deploy to UAT', script: '''
            ...
            ssh -o StrictHostKeyChecking=no ubuntu@${UAT_HOST} \\
              "CONTAINER='${CONTAINER}' MI_CARBONAPPS_DIR='${MI_CARBONAPPS_DIR}' REMOTE_STAGE_DIR='${REMOTE_STAGE_DIR}' SMOKE_URL='${SMOKE_URL}' bash -s" <<'EOS'
              set -euo pipefail
